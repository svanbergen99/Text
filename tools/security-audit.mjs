...
const forbiddenFilenamePatterns = [
  ...
  [/\.map$/i, "source map"], [/(^|\/)\.DS_Store$/i, "OS metadata"]
];
const sensitiveContentPatterns = [
  ["private key", /-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----/], ["AWS access key", /\bAKIA[0-9A-Z]{16}\b/],
  ["GitHub token", /\bgh[pousr]_[A-Za-z0-9_]{20,}\b/], ["Slack token", /\bxox[baprs]-[A-Za-z0-9-]{10,}\b/],
  ["Google API key", /\bAIza[0-9A-Za-z_-]{35}\b/], ["Stripe secret", /\bsk_(?:live|test)_[A-Za-z0-9]{16,}\b/],
  ["OpenAI-style secret", /\bsk-[A-Za-z0-9_-]{20,}\b/], ["JWT-like token", /\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\b/],
...
const riskyCodePatterns = [
  ["dynamic code execution", /\beval\s*\(|\bnew\s+Function\s*\(/], ["child-process execution", /(?:node:)?child_process|from\s+["']child_process["']|require\s*\(\s*["']child_process["']\s*\)/],
  ["unsafe browser HTML sink", /\.(?:innerHTML|outerHTML)\s*=|insertAdjacentHTML\s*\(/], ["document.write", /\bdocument\.write\s*\(/],
...
async function walk(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    const absolutePath = resolve(directory, entry.name); const path = relative(ROOT, absolutePath).replaceAll("\\", "/"); const info = await lstat(absolutePath);
    if (info.isSymbolicLink()) output.push({ path, absolutePath, info, symlink: true }); else if (info.isDirectory()) output.push(...await walk(absolutePath)); else if (info.isFile()) output.push({ path, absolutePath, info, symlink: false });
  }
  return output;
}
const findings = []; const files = await walk(ROOT);
for (const { path, absolutePath, info, symlink } of files) {
  ...
  const buffer = await readFile(absolutePath); if (buffer.includes(0)) { findings.push(`${path}: unexpected NUL/binary content in text file`); continue; }
  const text = buffer.toString("utf8"); if (text.includes("\uFFFD")) findings.push(`${path}: invalid UTF-8 replacement character detected`);
