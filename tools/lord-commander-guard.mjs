...
const sensitivePatterns = [
  ...
  /\b(?:password|passwd|secret|token|api[_-]?key|client[_-]?secret)\b\s*[:=]\s*["'`][^"'`\n]{8,}["'`]/i,
...
for (const [path, expected] of CRITICAL_CONTROLS) {
  try {
    ...
    if (actual !== expected) findings.push(`${path}: critical security control drifted`);
  } catch {
...
if (tags.length) findings.push(`unexpected Git tags present: ${tags.join(", ")}`);

const tracked = lines(git("ls-files"));
for (const path of tracked) {
  if (forbiddenNames.some((p) => p.test(path))) findings.push(`${path}: forbidden credential/archive filename`);
  if (SELF_FILES.has(path)) continue;
  const name = path.split("/").pop() || "";
  if (!TEXT_EXTENSIONS.has(extname(name).toLowerCase()) && !name.startsWith(".git")) continue;
  const data = await readFile(path);
  if (data.length > MAX_TEXT_BYTES) continue;
  if (data.includes(0)) continue;
  const text = data.toString("utf8");
  for (const pattern of sensitivePatterns) {
    if (pattern.test(text)) {
      findings.push(`${path}: possible secret or unsafe credential material`);
      break;
    }
  }
}

for (const row of lines(git("ls-files", "-s"))) {
  const match = row.match(/^(\d{6})\s+[0-9a-f]+\s+\d+\t(.+)$/);
  if (!match) continue;
  const [, mode, path] = match;
  if (mode !== "100644") findings.push(`${path}: unexpected Git file mode ${mode}`);
}
...
console.log(`Lord Commander guard passed: ${tracked.length} tracked files checked; critical security controls intact.`);
